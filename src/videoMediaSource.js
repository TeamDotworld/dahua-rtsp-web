...
function VideoMediaSource(a) {
  ...
  function f(a) {
    ...
    for (var b = 0; b < S.length; b++)
      a.addEventListener(S[b].type, S[b]["function"]);
  }
  ...
  function g() {
    ...
    if (null !== Q)
      for (a = 0; a < Q.length; a++)
        ab.removeEventListener(Q[a].type, Q[a]["function"]);
    if (null !== S)
      for (a = 0; a < S.length; a++)
        _.removeEventListener(S[a].type, S[a]["function"]);
    if (null !== R)
  ...
  function h() {
    if (null === _ || "ended" === _.readyState)
      return (
        (_ = new MediaSource()), f(_), ($.src = window.URL.createObjectURL(_))
      );
  ...
  function i(a) {
    if (null !== ab && "closed" !== _.readyState && "ended" !== _.readyState)
      try {
  ...
  function m() {
    if (null !== _)
      try {
        if (
          ab &&
          ab.buffered.length > 0 &&
          (l(),
          T ||
            ($.duration - $.currentTime > 1.5 &&
              ($.currentTime = ($.duration - 1).toFixed(3))),
          X)
        ) {
          var a = 1 * ab.buffered.start(ab.buffered.length - 1),
            ...
            c = 0;
          if (
            ((c = 0 === $.currentTime ? b - a : b - $.currentTime),
            c >= Z + 0.1)
          ) {
  ...
  function u() {
    ...
    0 === d ||
      ...
      (null === O
        ? (O = f)
        : ((O.timestamp <= f.timestamp && K >= 1) ||
            (O.timestamp > f.timestamp && 1 > K)) &&
          (P && W.timeStamp(f),
          (O = f),
          V++,
          V > a && H(f.timestamp, "currentTime")));
  }
  ...
  function y() {
    if (((X = !1), 0 == cb)) (bb = Date.now()), cb++;
      ...
      cb++;
      var a = Date.now() - bb;
      cb >= 5 && 6e4 > a && 1.8 >= Z && ((Z += 0.1), (cb = 0), (bb = 0));
    }
  }
  ...
  unknownB.prototype = {
    ...
    capture: function (a) {
      ...
      (b.width = $.videoWidth),
        ...
        X || "edge" === fb
          ? (b.getContext("2d").drawImage($, 0, 0, b.width, b.height),
            C(b.toDataURL(), a))
    ...
    terminate: function () {
      g(),
        "open" === _.readyState && ab && _.removeSourceBuffer(ab),
        (ab = null),
        _.endOfStream(),
        (ab = null),
        (_ = null),
        ($ = null),
        eb && (clearInterval(eb), (eb = null));
    },
